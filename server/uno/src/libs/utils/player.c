...
Player PlayerFromMap(Hashmap playermap, Connection con){
    ...
    player.username= *(String*)HashmapGet(&playermap,"username", converr);
    player.userPwd = *(String*)HashmapGet(&playermap,"userpwd", converr);
    player.imagepath = *(String*)HashmapGet(&playermap,"imagepath", converr);
...
List GetAlPlayers(Connection con){
    struct Arena scratch = ArenaCreate(1024);
    QueryResult res = ConnectionSelect(con,  "select * from player");
    ...
    for(int i = 0; i <playersTuples.size;i++){
        ...
        *player = PlayerFindById(StringToInt(*(String*)HashmapGet(&cardmap,"cardid"),converr),con)
...
QueryResult AddPlayerToGame(int playerid, int gameId, Connection con){
    return ConnectionExec(con, StringFormatChar("update player set joinedgameid = %d where playerid = %d", gameId, playerid));
};